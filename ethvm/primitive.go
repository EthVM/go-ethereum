// Code generated by github.com/actgardner/gogen-avro. DO NOT EDIT.
/*
 * SOURCE:
 *     schemas.asvc
 */

package ethvm

import (
	"fmt"
	"io"
	"math"
)

type ByteReader interface {
	ReadByte() (byte, error)
}

type ByteWriter interface {
	Grow(int)
	WriteByte(byte) error
}

type StringWriter interface {
	WriteString(string) (int, error)
}

func encodeInt(w io.Writer, byteCount int, encoded uint64) error {
	var err error
	var bb []byte
	bw, ok := w.(ByteWriter)
	// To avoid reallocations, grow capacity to the largest possible size
	// for this integer
	if ok {
		bw.Grow(byteCount)
	} else {
		bb = make([]byte, 0, byteCount)
	}

	if encoded == 0 {
		if bw != nil {
			err = bw.WriteByte(0)
			if err != nil {
				return err
			}
		} else {
			bb = append(bb, byte(0))
		}
	} else {
		for encoded > 0 {
			b := byte(encoded & 127)
			encoded = encoded >> 7
			if !(encoded == 0) {
				b |= 128
			}
			if bw != nil {
				err = bw.WriteByte(b)
				if err != nil {
					return err
				}
			} else {
				bb = append(bb, b)
			}
		}
	}
	if bw == nil {
		_, err := w.Write(bb)
		return err
	}
	return nil

}

func readArrayBytes(r io.Reader) ([][]byte, error) {
	var err error
	var blkSize int64
	var arr = make([][]byte, 0)
	for {
		blkSize, err = readLong(r)
		if err != nil {
			return nil, err
		}
		if blkSize == 0 {
			break
		}
		if blkSize < 0 {
			blkSize = -blkSize
			_, err = readLong(r)
			if err != nil {
				return nil, err
			}
		}
		for i := int64(0); i < blkSize; i++ {
			elem, err := readBytes(r)
			if err != nil {
				return nil, err
			}
			arr = append(arr, elem)
		}
	}
	return arr, nil
}

func readArrayLog(r io.Reader) ([]*Log, error) {
	var err error
	var blkSize int64
	var arr = make([]*Log, 0)
	for {
		blkSize, err = readLong(r)
		if err != nil {
			return nil, err
		}
		if blkSize == 0 {
			break
		}
		if blkSize < 0 {
			blkSize = -blkSize
			_, err = readLong(r)
			if err != nil {
				return nil, err
			}
		}
		for i := int64(0); i < blkSize; i++ {
			elem, err := readLog(r)
			if err != nil {
				return nil, err
			}
			arr = append(arr, elem)
		}
	}
	return arr, nil
}

func readArrayString(r io.Reader) ([]string, error) {
	var err error
	var blkSize int64
	var arr = make([]string, 0)
	for {
		blkSize, err = readLong(r)
		if err != nil {
			return nil, err
		}
		if blkSize == 0 {
			break
		}
		if blkSize < 0 {
			blkSize = -blkSize
			_, err = readLong(r)
			if err != nil {
				return nil, err
			}
		}
		for i := int64(0); i < blkSize; i++ {
			elem, err := readString(r)
			if err != nil {
				return nil, err
			}
			arr = append(arr, elem)
		}
	}
	return arr, nil
}

func readArrayTrace(r io.Reader) ([]*Trace, error) {
	var err error
	var blkSize int64
	var arr = make([]*Trace, 0)
	for {
		blkSize, err = readLong(r)
		if err != nil {
			return nil, err
		}
		if blkSize == 0 {
			break
		}
		if blkSize < 0 {
			blkSize = -blkSize
			_, err = readLong(r)
			if err != nil {
				return nil, err
			}
		}
		for i := int64(0); i < blkSize; i++ {
			elem, err := readTrace(r)
			if err != nil {
				return nil, err
			}
			arr = append(arr, elem)
		}
	}
	return arr, nil
}

func readArrayTransfer(r io.Reader) ([]*Transfer, error) {
	var err error
	var blkSize int64
	var arr = make([]*Transfer, 0)
	for {
		blkSize, err = readLong(r)
		if err != nil {
			return nil, err
		}
		if blkSize == 0 {
			break
		}
		if blkSize < 0 {
			blkSize = -blkSize
			_, err = readLong(r)
			if err != nil {
				return nil, err
			}
		}
		for i := int64(0); i < blkSize; i++ {
			elem, err := readTransfer(r)
			if err != nil {
				return nil, err
			}
			arr = append(arr, elem)
		}
	}
	return arr, nil
}

func readBlock(r io.Reader) (*Block, error) {
	var str = &Block{}
	var err error
	str.Number, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.Hash, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.ParentHash, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.IsUncle, err = readBool(r)
	if err != nil {
		return nil, err
	}
	str.IsCanonical, err = readBool(r)
	if err != nil {
		return nil, err
	}
	str.Timestamp, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.Nonce, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.MixDigest, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.Sha3Uncles, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.LogsBloom, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.StateRoot, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.TransactionsRoot, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.Miner, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.Difficulty, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.TotalDifficulty, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.ExtraData, err = readBytes(r)
	if err != nil {
		return nil, err
	}
	str.Size, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.GasLimit, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.GasUsed, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.TxsFees, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.BlockReward, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.UncleReward, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.Uncles, err = readArrayString(r)
	if err != nil {
		return nil, err
	}

	return str, nil
}

func readBool(r io.Reader) (bool, error) {
	var b byte
	var err error
	if br, ok := r.(ByteReader); ok {
		b, err = br.ReadByte()
	} else {
		bs := make([]byte, 1)
		_, err = io.ReadFull(r, bs)
		if err != nil {
			return false, err
		}
		b = bs[0]
	}
	return b == 1, nil
}

func readBytes(r io.Reader) ([]byte, error) {
	size, err := readLong(r)
	if err != nil {
		return nil, err
	}
	bb := make([]byte, size)
	_, err = io.ReadFull(r, bb)
	return bb, err
}

func readInt(r io.Reader) (int32, error) {
	var v int
	buf := make([]byte, 1)
	for shift := uint(0); ; shift += 7 {
		if _, err := io.ReadFull(r, buf); err != nil {
			return 0, err
		}
		b := buf[0]
		v |= int(b&127) << shift
		if b&128 == 0 {
			break
		}
	}
	datum := (int32(v>>1) ^ -int32(v & 1))
	return datum, nil
}

func readLog(r io.Reader) (*Log, error) {
	var str = &Log{}
	var err error
	str.Address, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.Topics, err = readArrayBytes(r)
	if err != nil {
		return nil, err
	}
	str.Data, err = readBytes(r)
	if err != nil {
		return nil, err
	}
	str.Removed, err = readBool(r)
	if err != nil {
		return nil, err
	}

	return str, nil
}

func readLong(r io.Reader) (int64, error) {
	var v uint64
	buf := make([]byte, 1)
	for shift := uint(0); ; shift += 7 {
		if _, err := io.ReadFull(r, buf); err != nil {
			return 0, err
		}
		b := buf[0]
		v |= uint64(b&127) << shift
		if b&128 == 0 {
			break
		}
	}
	datum := (int64(v>>1) ^ -int64(v & 1))
	return datum, nil
}

func readNull(_ io.Reader) (interface{}, error) {
	return nil, nil
}

func readString(r io.Reader) (string, error) {
	len, err := readLong(r)
	if err != nil {
		return "", err
	}

	// makeslice can fail depending on available memory.
	// We arbitrarily limit string size to sane default (~2.2GB).
	if len < 0 || len > math.MaxInt32 {
		return "", fmt.Errorf("string length out of range: %d", len)
	}

	bb := make([]byte, len)
	_, err = io.ReadFull(r, bb)
	if err != nil {
		return "", err
	}
	return string(bb), nil
}

func readTrace(r io.Reader) (*Trace, error) {
	var str = &Trace{}
	var err error
	str.IsError, err = readBool(r)
	if err != nil {
		return nil, err
	}
	str.Msg, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.Transfers, err = readArrayTransfer(r)
	if err != nil {
		return nil, err
	}

	return str, nil
}

func readTransaction(r io.Reader) (*Transaction, error) {
	var str = &Transaction{}
	var err error
	str.Hash, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.Root, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.Index, err = readInt(r)
	if err != nil {
		return nil, err
	}
	str.Timestamp, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.Nonce, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.NonceHash, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.From, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.FromBalance, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.To, err = readUnionNullString(r)
	if err != nil {
		return nil, err
	}
	str.ToBalance, err = readUnionNullString(r)
	if err != nil {
		return nil, err
	}
	str.Input, err = readBytes(r)
	if err != nil {
		return nil, err
	}
	str.ContractAddress, err = readUnionNullString(r)
	if err != nil {
		return nil, err
	}
	str.Value, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.Gas, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.GasPrice, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.GasUsed, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.CumulativeGasUsed, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.V, err = readBytes(r)
	if err != nil {
		return nil, err
	}
	str.R, err = readBytes(r)
	if err != nil {
		return nil, err
	}
	str.S, err = readBytes(r)
	if err != nil {
		return nil, err
	}
	str.Status, err = readLong(r)
	if err != nil {
		return nil, err
	}
	str.LogsBloom, err = readBytes(r)
	if err != nil {
		return nil, err
	}
	str.Logs, err = readArrayLog(r)
	if err != nil {
		return nil, err
	}
	str.Traces, err = readArrayTrace(r)
	if err != nil {
		return nil, err
	}

	return str, nil
}

func readTransfer(r io.Reader) (*Transfer, error) {
	var str = &Transfer{}
	var err error
	str.Op, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.From, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.To, err = readString(r)
	if err != nil {
		return nil, err
	}
	str.Input, err = readBytes(r)
	if err != nil {
		return nil, err
	}

	return str, nil
}

func readUnionBlockTransactionLogTraceTransfer(r io.Reader) (UnionBlockTransactionLogTraceTransfer, error) {
	field, err := readLong(r)
	var unionStr UnionBlockTransactionLogTraceTransfer
	if err != nil {
		return unionStr, err
	}
	unionStr.UnionType = UnionBlockTransactionLogTraceTransferTypeEnum(field)
	switch unionStr.UnionType {
	case UnionBlockTransactionLogTraceTransferTypeEnumBlock:
		val, err := readBlock(r)
		if err != nil {
			return unionStr, err
		}
		unionStr.Block = val
	case UnionBlockTransactionLogTraceTransferTypeEnumTransaction:
		val, err := readTransaction(r)
		if err != nil {
			return unionStr, err
		}
		unionStr.Transaction = val
	case UnionBlockTransactionLogTraceTransferTypeEnumLog:
		val, err := readLog(r)
		if err != nil {
			return unionStr, err
		}
		unionStr.Log = val
	case UnionBlockTransactionLogTraceTransferTypeEnumTrace:
		val, err := readTrace(r)
		if err != nil {
			return unionStr, err
		}
		unionStr.Trace = val
	case UnionBlockTransactionLogTraceTransferTypeEnumTransfer:
		val, err := readTransfer(r)
		if err != nil {
			return unionStr, err
		}
		unionStr.Transfer = val

	default:
		return unionStr, fmt.Errorf("Invalid value for UnionBlockTransactionLogTraceTransfer")
	}
	return unionStr, nil
}

func readUnionNullString(r io.Reader) (UnionNullString, error) {
	field, err := readLong(r)
	var unionStr UnionNullString
	if err != nil {
		return unionStr, err
	}
	unionStr.UnionType = UnionNullStringTypeEnum(field)
	switch unionStr.UnionType {
	case UnionNullStringTypeEnumNull:
		val, err := readNull(r)
		if err != nil {
			return unionStr, err
		}
		unionStr.Null = val
	case UnionNullStringTypeEnumString:
		val, err := readString(r)
		if err != nil {
			return unionStr, err
		}
		unionStr.String = val

	default:
		return unionStr, fmt.Errorf("Invalid value for UnionNullString")
	}
	return unionStr, nil
}

func writeArrayBytes(r [][]byte, w io.Writer) error {
	err := writeLong(int64(len(r)), w)
	if err != nil || len(r) == 0 {
		return err
	}
	for _, e := range r {
		err = writeBytes(e, w)
		if err != nil {
			return err
		}
	}
	return writeLong(0, w)
}

func writeArrayLog(r []*Log, w io.Writer) error {
	err := writeLong(int64(len(r)), w)
	if err != nil || len(r) == 0 {
		return err
	}
	for _, e := range r {
		err = writeLog(e, w)
		if err != nil {
			return err
		}
	}
	return writeLong(0, w)
}

func writeArrayString(r []string, w io.Writer) error {
	err := writeLong(int64(len(r)), w)
	if err != nil || len(r) == 0 {
		return err
	}
	for _, e := range r {
		err = writeString(e, w)
		if err != nil {
			return err
		}
	}
	return writeLong(0, w)
}

func writeArrayTrace(r []*Trace, w io.Writer) error {
	err := writeLong(int64(len(r)), w)
	if err != nil || len(r) == 0 {
		return err
	}
	for _, e := range r {
		err = writeTrace(e, w)
		if err != nil {
			return err
		}
	}
	return writeLong(0, w)
}

func writeArrayTransfer(r []*Transfer, w io.Writer) error {
	err := writeLong(int64(len(r)), w)
	if err != nil || len(r) == 0 {
		return err
	}
	for _, e := range r {
		err = writeTransfer(e, w)
		if err != nil {
			return err
		}
	}
	return writeLong(0, w)
}

func writeBlock(r *Block, w io.Writer) error {
	var err error
	err = writeLong(r.Number, w)
	if err != nil {
		return err
	}
	err = writeString(r.Hash, w)
	if err != nil {
		return err
	}
	err = writeString(r.ParentHash, w)
	if err != nil {
		return err
	}
	err = writeBool(r.IsUncle, w)
	if err != nil {
		return err
	}
	err = writeBool(r.IsCanonical, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Timestamp, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Nonce, w)
	if err != nil {
		return err
	}
	err = writeString(r.MixDigest, w)
	if err != nil {
		return err
	}
	err = writeString(r.Sha3Uncles, w)
	if err != nil {
		return err
	}
	err = writeString(r.LogsBloom, w)
	if err != nil {
		return err
	}
	err = writeString(r.StateRoot, w)
	if err != nil {
		return err
	}
	err = writeString(r.TransactionsRoot, w)
	if err != nil {
		return err
	}
	err = writeString(r.Miner, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Difficulty, w)
	if err != nil {
		return err
	}
	err = writeLong(r.TotalDifficulty, w)
	if err != nil {
		return err
	}
	err = writeBytes(r.ExtraData, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Size, w)
	if err != nil {
		return err
	}
	err = writeLong(r.GasLimit, w)
	if err != nil {
		return err
	}
	err = writeLong(r.GasUsed, w)
	if err != nil {
		return err
	}
	err = writeLong(r.TxsFees, w)
	if err != nil {
		return err
	}
	err = writeLong(r.BlockReward, w)
	if err != nil {
		return err
	}
	err = writeLong(r.UncleReward, w)
	if err != nil {
		return err
	}
	err = writeArrayString(r.Uncles, w)
	if err != nil {
		return err
	}

	return nil
}

func writeBool(r bool, w io.Writer) error {
	var b byte
	if r {
		b = byte(1)
	}

	var err error
	if bw, ok := w.(ByteWriter); ok {
		err = bw.WriteByte(b)
	} else {
		bb := make([]byte, 1)
		bb[0] = b
		_, err = w.Write(bb)
	}
	if err != nil {
		return err
	}
	return nil
}

func writeBytes(r []byte, w io.Writer) error {
	err := writeLong(int64(len(r)), w)
	if err != nil {
		return err
	}
	_, err = w.Write(r)
	return err
}

func writeInt(r int32, w io.Writer) error {
	downShift := uint32(31)
	encoded := uint64((uint32(r) << 1) ^ uint32(r>>downShift))
	const maxByteSize = 5
	return encodeInt(w, maxByteSize, encoded)
}

func writeLog(r *Log, w io.Writer) error {
	var err error
	err = writeString(r.Address, w)
	if err != nil {
		return err
	}
	err = writeArrayBytes(r.Topics, w)
	if err != nil {
		return err
	}
	err = writeBytes(r.Data, w)
	if err != nil {
		return err
	}
	err = writeBool(r.Removed, w)
	if err != nil {
		return err
	}

	return nil
}

func writeLong(r int64, w io.Writer) error {
	downShift := uint64(63)
	encoded := uint64((r << 1) ^ (r >> downShift))
	const maxByteSize = 10
	return encodeInt(w, maxByteSize, encoded)
}

func writeNull(_ interface{}, _ io.Writer) error {
	return nil
}

func writeString(r string, w io.Writer) error {
	err := writeLong(int64(len(r)), w)
	if err != nil {
		return err
	}
	if sw, ok := w.(StringWriter); ok {
		_, err = sw.WriteString(r)
	} else {
		_, err = w.Write([]byte(r))
	}
	return err
}

func writeTrace(r *Trace, w io.Writer) error {
	var err error
	err = writeBool(r.IsError, w)
	if err != nil {
		return err
	}
	err = writeString(r.Msg, w)
	if err != nil {
		return err
	}
	err = writeArrayTransfer(r.Transfers, w)
	if err != nil {
		return err
	}

	return nil
}
func writeTransaction(r *Transaction, w io.Writer) error {
	var err error
	err = writeString(r.Hash, w)
	if err != nil {
		return err
	}
	err = writeString(r.Root, w)
	if err != nil {
		return err
	}
	err = writeInt(r.Index, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Timestamp, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Nonce, w)
	if err != nil {
		return err
	}
	err = writeString(r.NonceHash, w)
	if err != nil {
		return err
	}
	err = writeString(r.From, w)
	if err != nil {
		return err
	}
	err = writeLong(r.FromBalance, w)
	if err != nil {
		return err
	}
	err = writeUnionNullString(r.To, w)
	if err != nil {
		return err
	}
	err = writeUnionNullString(r.ToBalance, w)
	if err != nil {
		return err
	}
	err = writeBytes(r.Input, w)
	if err != nil {
		return err
	}
	err = writeUnionNullString(r.ContractAddress, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Value, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Gas, w)
	if err != nil {
		return err
	}
	err = writeLong(r.GasPrice, w)
	if err != nil {
		return err
	}
	err = writeLong(r.GasUsed, w)
	if err != nil {
		return err
	}
	err = writeLong(r.CumulativeGasUsed, w)
	if err != nil {
		return err
	}
	err = writeBytes(r.V, w)
	if err != nil {
		return err
	}
	err = writeBytes(r.R, w)
	if err != nil {
		return err
	}
	err = writeBytes(r.S, w)
	if err != nil {
		return err
	}
	err = writeLong(r.Status, w)
	if err != nil {
		return err
	}
	err = writeBytes(r.LogsBloom, w)
	if err != nil {
		return err
	}
	err = writeArrayLog(r.Logs, w)
	if err != nil {
		return err
	}
	err = writeArrayTrace(r.Traces, w)
	if err != nil {
		return err
	}

	return nil
}
func writeTransfer(r *Transfer, w io.Writer) error {
	var err error
	err = writeString(r.Op, w)
	if err != nil {
		return err
	}
	err = writeString(r.From, w)
	if err != nil {
		return err
	}
	err = writeString(r.To, w)
	if err != nil {
		return err
	}
	err = writeBytes(r.Input, w)
	if err != nil {
		return err
	}

	return nil
}

func writeUnionBlockTransactionLogTraceTransfer(r UnionBlockTransactionLogTraceTransfer, w io.Writer) error {
	err := writeLong(int64(r.UnionType), w)
	if err != nil {
		return err
	}
	switch r.UnionType {
	case UnionBlockTransactionLogTraceTransferTypeEnumBlock:
		return writeBlock(r.Block, w)
	case UnionBlockTransactionLogTraceTransferTypeEnumTransaction:
		return writeTransaction(r.Transaction, w)
	case UnionBlockTransactionLogTraceTransferTypeEnumLog:
		return writeLog(r.Log, w)
	case UnionBlockTransactionLogTraceTransferTypeEnumTrace:
		return writeTrace(r.Trace, w)
	case UnionBlockTransactionLogTraceTransferTypeEnumTransfer:
		return writeTransfer(r.Transfer, w)

	}
	return fmt.Errorf("Invalid value for UnionBlockTransactionLogTraceTransfer")
}

func writeUnionNullString(r UnionNullString, w io.Writer) error {
	err := writeLong(int64(r.UnionType), w)
	if err != nil {
		return err
	}
	switch r.UnionType {
	case UnionNullStringTypeEnumNull:
		return writeNull(r.Null, w)
	case UnionNullStringTypeEnumString:
		return writeString(r.String, w)

	}
	return fmt.Errorf("Invalid value for UnionNullString")
}
